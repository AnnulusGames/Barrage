...
namespace Barrage.Unity.Editor
{
    ...
    public sealed class EntityEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var archetype = proxy.world.GetEntityArchetype(proxy.entity);

            foreach (var componentType in archetype.GetComponentTypes())
            {
                var component = proxy.world.GetComponent(proxy.entity, componentType.Type);
                if (component is GameObject or EntityConverter or EntityName) continue;

                if (!isExpandedDictionary.TryGetValue(componentType.Type.FullName, out var isExpanded))
                {
                    isExpandedDictionary.Add(componentType.Type.FullName, true);
                    isExpanded = true;
                }

                var headerText = ObjectNames.NicifyVariableName(componentType.Type.Name);
                var headerIconImage = component is Component co
                    ? EditorGUIUtility.ObjectContent(co, componentType.Type).image
                ...
                if (isExpanded)
                {
                    ...
                    using (new EditorGUI.IndentLevelScope())
                    {
                        ...
                        {
                            DrawMembers(component, componentType.Type.FullName, 0);
                        }
                    }
                }
                isExpandedDictionary[componentType.Type.FullName] = isExpanded;
        ...
        static void DrawHeader(EntitySelectionProxy selectionProxy)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    var hasGameObject = selectionProxy.world.TryGetComponent(entity, out GameObject gameObjectReference);
                    var hasName = selectionProxy.world.TryGetComponent(entity, out EntityName entityName);

                    using (new EditorGUILayout.HorizontalScope())
                    {
                        EditorGUILayout.TextField(hasName ? entityName.ToString() : $"Entity({entity.Index}:{entity.Version})");

                        EditorGUI.indentLevel++;
                        EditorGUIUtility.labelWidth = 15f;
                        EditorGUILayout.IntField("Id", entity.Index, GUILayout.Width(80f));
                    ...
                    EditorGUILayout.ObjectField("From", hasGameObject ? gameObjectReference : null, typeof(GameObject), true);
                }
            }
